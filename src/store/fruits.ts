...
export const useFruitsStore = defineStore('fruits',()=>{  //组合式
    //相当于state,数据
    const fruitsList = reactive(
    JSON.parse(localStorage.getItem('fruits') as string) || []
    )
    //相当于actions,方法
    function getAFruit(Fruits_basket){
        //获取对象
        let obj = Fruits_basket.pop()
        //包装为fruit
        if(obj){  //判断是否还有剩余的水果对象
            let fruit = {id:nanoid(),title:obj.title}
            fruitsList.unshift(fruit)  //后来的放前面
        }else{
            console.log("没有水果了")
        }
    }
    return {fruitsList,getAFruit}  //要交出内容
})